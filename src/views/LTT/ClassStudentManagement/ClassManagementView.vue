            ...
            <el-row>
                ...
                <el-col :span="10">
                    <el-form-item label="结课时间">
                        <el-date-picker
                            v-model="clsSearchData.entrydate"
                            type="daterange"
                            range-separator="至"
...
export default {
    data() {
        return {
            ...
            // 显示班主任区数据
            showTeacherData: [], // 用于存储班级数据
            teacherNames: {}, // 用于存储班级 ID 和名称的映射
            loadingTeacherNames: {}, // 用于跟踪哪些班级 ID 的请求正在进行
        };
    },
    methods: {
        // 分页控制区方法
        ...
        teacherNameFormatter(row) {
            if (this.teacherNames[row.teacher]) {
                return this.teacherNames[row.teacher];
            } else {
                this.loadTeacherName(row.teacher);
                return "加载中...";
            }
        },
